...
- 出现一些报错，例如如下错误，可以尝试在服务器复现，复现不了的话可以先注释 test_crossformer_compatibility。如果 Bot 被取消了并且 ci 错误和自己的 pr 无关，需要重新点 bot 请求 ci，以及如果 pr 很久都没有被调度到可以尝试取消 bot，然后刷新网页重新点一下 bot 
